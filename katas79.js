...
describe('`catch()` returns a Promise and deals with rejected cases only', () => {
    ...
    describe('prerequisites for understanding', () => {
      ...
      it('*return* a fulfilled promise, to pass a test', () => {
        return Promise.resolve(); //added "return" in front of Promise.resolve();
        //in the past: Promise.resolve();
      ...
      it('reminder: the test passes when a fulfilled promise is returned', () => {
        ...
        //in the past: return Promise.reject('I should fulfill.');
      });
    ...
    describe('`catch` method basics', () => {
      it('is an instance method', () => {
        ...
        //added .resolve() after Promise
        //in the past: const p = Promise;
        assert.equal(typeof p.catch, 'function');
      });
  
      it('catches only promise rejections', (done) => {
        const promise = Promise.reject();
        //changed Promise.resolve() to Promise.reject()
      ...
      it('returns a new promise', () => {
        const whatToReturn = () => Promise.resolve();//changed Promise.reject() to Promise.resolve()
        // in the past: const whatToReturn = () => Promise.reject();
        const promise = Promise.reject();
      ...
      it('converts it`s return value into a promise', () => {
        ...
        //changed void 0 to 'promise?'
        //in the past: const p1 = p.catch(() => void 0);
      ...
      it('the first parameter is the rejection reason', () => {
        ...
        //changed 'rejection' to 'oops'
        //in the past: const p = Promise.reject('rejection');
    ...
    describe('multiple `catch`es', () => {
      it('only the first `catch` is called', () => {
        ...
        const p1 = p
            .catch(reason => `${reason} AND 2`)//changed void 0 to `${reason} AND 2`
            //in the past: .catch(reason => void 0)
      ...
      it('if a `catch` throws, the next `catch` catches it', () => {
        ...
        const p1 = p
            ...
            //in the past: .catch(err => { throw Error (`${err.message} AND 3`) })
