...
describe('`map.has()` indicates whether an element with a key exists', function() {
    ...
    it('finds nothing in an empty map', function() {
      let map = new Map();
      const hasKey = map.has(void 0); //fixed hazz to has
      //in the past: const hasKey = map.hazz(void 0);
    ...
    it('finds an element by it`s key', function() {
      ...
      //added 'key' inside map.has()
      //in the past: const hasKey = map.has();
    ...
    it('finds `undefined` as key too', function() {
      ...
      //added .has()
      //in the past: const hasUndefinedAsKey = map;
    ...
    it('does not coerce keys', function() {
      ...
      //in the past: const findsStringOne = true;
    ...
    it('after removal (using `map.delete(<key>)`) it doesnt find the element anymore', function() {
      let map = new Map([[1, 'one']]);
      map.delete(1);
      //added map.delete(1) so that map no longer has 1 and still has 'one'
